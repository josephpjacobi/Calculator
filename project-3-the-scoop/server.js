...
function createComment(url, request) {
   ...
   const response = {};
    if (request.body && newComment && newComment.username && database.users[newComment.username] && database.articles[newComment.articleId]
      && newComment.body && newComment.articleId
      ) {
      newComment.id = database.nextCommentId;
      newComment.upvotedBy = [];
      newComment.downvotedBy = [];
...
function updatedComment(url, request) { 
  const response = {};
  ...
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const updatedComment = request.body.comment;
  ...
  if(updatedComment.body) {
    database.comments[id].body  = updatedComment.body;
    response.status = 200;
    return response;
  } 
...
function deleteComment(url) {
  ...
  if (database.comments[id]) {
    const author = database.comments[id].username;
    const article = database.comments[id].articleId;
    const authorCommentIndex = database.users[author].commentIds.indexOf(id);
    database.users[author].commentIds.splice(authorCommentIndex, 1);
    const articleCommentIndex = database.articles[article].commentIds.indexOf(id);
    database.articles[article].commentIds.splice(articleCommentIndex, 1);
...
function addUpVote(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const response = {};

  if (request.body && request.body.username) {
  const upVoteUser = request.body.username;
  
  if (database.users[upVoteUser] && database.comments[id]) {
    const comment = database.comments[id];
    if (!comment.upvotedBy.includes(upVoteUser)) {
      comment.upvotedBy.push(upVoteUser);
      }
    if (comment.downvotedBy.includes(upVoteUser)) {
      comment.downvotedBy.splice(comment.downvotedBy.indexOf(upVoteUser), 1)
    }
...
function addDownVote(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const response = {};

  if (request.body && request.body.username) {
  const downVoteUser = request.body.username;
  
  if (database.users[downVoteUser] && database.comments[id]) {
    const comment = database.comments[id];
    if (!comment.downvotedBy.includes(downVoteUser)) {
      comment.downvotedBy.push(downVoteUser);
      }
    if (comment.upvotedBy.includes(downVoteUser)) {
      comment.upvotedBy.splice(comment.upvotedBy.indexOf(downVoteUser), 1)
    }
